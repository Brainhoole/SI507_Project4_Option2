    ...
    def deflect_ball(self, ball, side_hit):
        ...
        if side_hit:
            ...
            if (self.game.hit_count%10)==0:
                self.game.balls[0].velocity += 1
            self.game.game_objects.remove(self)

class Brick_Lastline(BallDeflector):
    def deflect_ball(self, ball, side_hit):
        BallDeflector.deflect_ball(self, ball, side_hit)
        if side_hit:
            self.game.increment_hit_count()
            if (self.game.hit_count%10)==0:
                self.game.balls[0].velocity += 1
            self.game.balls[0].velocity -= 0.5
            self.game.game_objects.remove(self)
...
class Ball(GameObject):
    ...
    default_velocity = 6 #Number of pixels the ball should move per game cycle
...
class Game(object):
    ...
    def __init__(self,
        ...
        # self.balls[0].velocity = self.balls[0].default_velocity + 5*(self.hit_count//2)
        self.paddles = [
            Paddle(player = 1,
        ...
        for i in range(0,11):
            for j in range(0,5):
                self.bricks.append(Brick(initial_x = self.width-6*brick_height-30+j*(brick_height+1),
                initial_y = i*(brick_height+1),
                img_file = wall_imgs[2],
                game = self))
            self.bricks.append(Brick_Lastline(initial_x = self.width-6*brick_height-30+5*(brick_height+1),
            initial_y = i*(brick_height+1),
            img_file = wall_imgs[3],
            game = self))
...
def main():
    ...
    wall_imgs = [pyglet.resource.image('vertical_wall.png'),
                 ...
                 pyglet.resource.image('brick_Lastline.png')]
